...
const Festival = () => {
  ...
  const [campaigId, setCampaignId] = useState('');
  ...
  const fetchDatas = async () => {
    try {
      const response = await HomepageService.festivalItems();
      setFreeShipping(response.data.products.products); 
      setTitle(response.data.products.campaign.title)
      setCampaignId(response.data.products.campaign._id);
    } catch (error) {
  ...
  const handleClick = () => {
    ...
    const url = `source?source_module=${encodeURIComponent(
      title.replace(/ /g, "-")
    )}.html&source_identifier=${campaigId}&target=most-popular&page=1`;
