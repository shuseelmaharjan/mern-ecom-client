...
const CompanyPolicies = () => {
  ...
  useEffect(() => {
    const getCompanyPolicy = async () => {
      try {
        const response = await sitePolicyService.getSitePolicyData();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await sitePolicyService.createSitePolicy(
        formData,
        accessToken
      );
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const response = await sitePolicyService.updatePolicy(policyId, updatedPolicy, accessToken);
      ...
      const updatedPolicies = await sitePolicyService.getSitePolicyData();
  ...
  return (
    <>
      <div className="w-full p-4 bg-yellow-100 border-yellow-500 border-2 mb-4">
        Here you should add company policy data.
      </div>
